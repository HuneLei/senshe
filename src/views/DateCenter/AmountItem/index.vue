<!-- 四级界面数量详情 -->
<template>
  <div class="scroller_rela" ref="amountItem">
    <x-table :cell-bordered="true" class="table_thead" ref="AmountItemThead" id="AmountItemThead">
      <thead>
        <tr style="background-color: #F8F8F8">
          <th class="table_thead_longth">客户名称</th>
          <th class="table_tbody_medth">月份</th>
          <th class="table_tbody_medth">数量</th>
          <th class="table_tbody_medth">金额</th>
        </tr>
      </thead>
    </x-table>
    <scroller ref="AmountItemScroller" :on-refresh="refresh" :noDataText='noDataText' refreshText='下拉刷新'>
        ...
        <x-table :cell-bordered="true">
          <tbody class="table_tbody">
            <tr v-for="(item, index) in indexList" :key="index">
              ...
              <td class="table_tbody_medth">{{item.month}}</td>
              <td class="table_tbody_medth">{{item.num}}</td>
...
export default {
  ...
  activated() {
    this.indexList = [];
    this.$refs.AmountItemScroller.triggerPullToRefresh()
  },
  mounted() {
    const that = this;
    // 屏幕高度设置
    this.$nextTick(() => {
      const Top = document.querySelector('#AmountItemThead').clientHeight;
      that.$refs.AmountItemScroller.$el.style.top = `${Top}px`;
      that.$refs.AmountItemScroller.$el.style.height = `${that.$countHeight(['.vux-header', '#AmountItemThead'])}px`;
    })
  },
  computed: {},
  components: {},
